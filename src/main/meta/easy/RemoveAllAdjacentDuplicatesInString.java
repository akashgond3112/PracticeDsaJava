...
public class RemoveAllAdjacentDuplicatesInString {
	 ...
	 *
	 * #### **Time Complexity:**
	 * - We traverse the string **once** (`O(n)`) and for each character, we either **append** or **delete** from `StringBuilder`, which takes **O(1)** time per operation.
	 * - Thus, the overall **time complexity is O(n)**.
	 *
	 * #### **Space Complexity:**
	 * - The worst-case space complexity occurs when there are **no duplicates**, meaning we store the entire string in `StringBuilder`, which takes **O(n) space**.
	...
	/**
	 * Solution for removing consecutive duplicate characters from a string using recursion.
	 * For example: "aabb" -> "ab", "aabbcc" -> "abc"
	 */
	static class SolutionUsingRecursion {
		 ...
		 *
		 * @param s Input string from which duplicates need to be removed
